...
add_action('plugins_loaded', 'woocommerce_demopayment_generic_init', 0);

function woocommerce_demopayment_generic_init() {
    if (!class_exists('WC_Payment_Gateway'))
        return;

    class WC_demopaymentGeneric extends WC_Payment_Gateway {

        public function __construct() {
            
            $this->id = 'demopayment_generic';
            $this->has_fields = false;
            $this->method_title = 'Demo Payment Gateway';
            ...
            add_action('woocommerce_receipt_demopayment_generic', array(&$this, 'receipt_page'));

            /* callback/datafeed */
            add_action('woocommerce_api_wc_demopayment', array($this, 'gateway_response'));
        ...
        function init_form_fields() {
            ...
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('Enable Demo Payment Generic Module.'),
                    'default' => 'no'),
                'title' => array(
                    'title' => __('Title:'),
                    'type' => 'text',
                    ...
                    'default' => __('Demo Payment Gateway')),
                'description' => array(
                    'title' => __('Description:'),
                    'type' => 'textarea',
                    'required' => true,
                    'description' => __('The description to be shown in checkout.'),
                    'default' => __('Pay securely through Demo Payment Generic services.')),
                'gateway_url' => array(
        ...
        public function admin_options() {
            ...
            $str = '<h3>Demo Payment Generic Gateway</h3>
                    <p>
                        <strong>Gateway URL:</strong> <br/>
                        - Production: https://payment.awoz.co/app/page/generic/ <br/>
                        - Sandbox Test: https://payment-sandbox.awoz.co/app/page/generic/ <br/>
                        <br/>
                    </p>
                    <hr/>
...
//            include 'redirect.js';
            ...
            $fields = $this->generate_demopayment_form($order);
            $payment_array = array();
...
//            echo $post_js;
         ...
         * */
        public function generate_demopayment_form($order_id) {
            global $woocommerce, $_SERVER;

            $order = new WC_Order($order_id);
            
            $return_url = $this->shop_url."/index.php?wc-api=wc_pa_success_page";
            $notify_url = $this->shop_url."/index.php?wc-api=wc_demopayment";
...
//            include 'success_redirect.js';
     ...
     * */
    function woocommerce_add_demopayment_generic_gateway($methods) {
        $methods[] = 'WC_demopaymentGeneric';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'woocommerce_add_demopayment_generic_gateway');
}
